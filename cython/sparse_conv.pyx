...
cdef extern from "../cpp/convolutions.cpp" namespace "conv_fl32":
    cdef cppclass ConvolutionCalculator:
        void prepare_memory(my_cfl32* PSF_subpx, int64_t psf_res, int64_t subpixels, int verbose)
        ...
        void async_convolve(int number_of_threads, int64_t sample_count, int64_t particle_count, int64_t step_count, int64_t camera_fov_px, int * subpx_poss, int * offpx_poss, int * sample_sizes, my_cfl32 * intensities, int verbose)
...
cdef class ConvolutionCalculator_fl32:
    ...
    def __init__(self, np.ndarray[my_fl32, ndim=4] PSF_subpx, verbose = 0):
        ...
        self.conv_calc.prepare_memory(<my_cfl32*>&PSF_subpx[0, 0, 0, 0], self.psf_res, self.subpixels, verbose)
